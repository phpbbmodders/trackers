...
class search
{
	...
	public function display()
	{
		...
		$project_id = $this->request->variable('project_id', -1);
		$status_id = $this->request->variable('status_id', 0);
		$severity_id = $this->request->variable('severity_id', -1);
		$assigned_user = $this->request->variable('assigned_user', -1);
		$assigned_group = $this->request->variable('assigned_group', -1);
		$include_groups = $this->request->variable('include_groups', false);

		$scope_title = $this->request->variable('scope_title', false);
		$scope_report = $this->request->variable('scope_report', false);
		$scope_replies = $this->request->variable('scope_replies', false);

		$this->language->add_lang('search', 'phpbbmodders/trackers');
